...
Runs as one instance per ATS target (see deployment model in `../README.md`) — each running instance is configured with which ATS to target, a role/keyword filter (e.g. "python positions" vs "go positions"), and a run-duration after which it times out. The role filter only narrows what sourcing pulls in; it doesn't change matching or tailoring behavior downstream.
