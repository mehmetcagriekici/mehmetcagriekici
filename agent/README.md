...
**Matching is fully deterministic — no LLM in the fit decision (updated 2026-07-22).** `hybrid_search` (structured field-diff + embedding/keyword search, sentence-transformers embeddings, CPU-only, cosine similarity) runs as a two-stage sequential gate: `job_config` vs. `job_posting` first (cheap, per-instance parameter, not cached), then `source_of_truth` vs. `job_posting` only if stage 1 passes (`source_of_truth` embeddings precomputed/cached — the only embeddings that are). Soft/inferred requirements (e.g. "strong communication skills") are dropped rather than arbitrated — an accepted tradeoff for now. The LLM runs unconditionally on everything that clears both gates, but only to generate the application materials — it has no say in the fit decision. See `matching/README.md`.
