...
Job-application automation project — design-first, implementation code written only where explicitly requested. An AI agent that will apply to jobs on the user's behalf. `source_of_truth/` and `matching/` are built and in use; `tailoring/` is in progress; `sourcing/`, `form_automation/`, `tracking/`, and `review_gate/` are still planning stage, no code yet — see the module table below.
...
| [`tailoring/`](tailoring/README.md) | In progress — Ollama client, all three prompts, and Playwright PDF rendering with one-page detection built; HTML/CSS templates and review-routing on overflow not yet wired | Generates per-job resume/cover letter |
...
The one-folder-per-module split is holding up in practice — `tailoring/` has already split into `llm/`, `prompts/`, and `write/` submodules as it's been built out. Where generated per-job artifacts land is also decided: written to disk per application, deleted once that application concludes (sent, rejected, or answered after an overflow email) — see `CLAUDE.md`.
...
The LLM's role is constrained to organizing/formatting resume and cover-letter content and answering form fields strictly from `source_of_truth/`, never hallucinating facts not present there — same principle as the honesty rules in the root `CLAUDE.md`. `source_of_truth/` has expanded beyond `profile.json` to five files, all built and in use: a projects-detail file (incident-level material per project), a general-stories file (non-project-specific anecdotes), a hand-maintained known-gaps file (honest phrasing for this candidate's recurring gaps), and a preferences file (the shared role/location/stack filter `matching/` searches against — see `source_of_truth/README.md`).
...
**Pipeline (decided 2026-07-22):** `Data -> Hybrid Search -> Application Generator -> Application Controller -> Application API`. `matching/` is Hybrid Search; `tailoring/` is the Application Generator, running unconditionally on every posting that clears the match step, consuming the top-N `(id, content)` `Document` pairs `hybrid_search` returns directly — no separate resolve-by-id step back to a richer structured record (revised 2026-08-04, see `matching/README.md`'s Types section). It makes three separate JSON-returning Ollama calls per posting (resume, cover letter, free-text answers), which `tailoring/write/` renders to PDF via Playwright with a one-page check. `review_gate/` is the Application Controller — a second LLM checking the generator's output for honesty (vs. `source_of_truth`) and relevance (vs. the job posting), any failure on either routing to human review with no severity distinction; `form_automation/` (bridged through Go) is the Application API, firing only once the Controller or the user approves.
